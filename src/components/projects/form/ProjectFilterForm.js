...
export default function ProjectFilterForm({ control, errors }) {
    return (
        <Grid container spacing={4} maxWidth="lg" className="projectForm !px-4 md:px-8 justify-center project-search-form gap-2 items-center">
            <Grid item xs={12} md={3} className="space-y-2 !p-0">
                <FormInput
                    name="projectSearch"
                    placeholder="Search..."
                    control={control}
            ...
            {Object.keys(selectOptions).map((key) => {
                return (
                    <Grid item xs={12} md={2} className="space-y-2 !p-0" key={key}>
                        <FormInputSelect
                            name={key}
            ...
            <Grid item xs={12} md={1} className="space-y-2 !p-0">
